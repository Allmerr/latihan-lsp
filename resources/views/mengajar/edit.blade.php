...
<div class="m2 form-mengajar-container text-white">
    <div class="form-mengajar rounded">
        <h2 class="mt2 text-center">Ubah mengajar</h2>
        <form action="{{ route('mengajar.update',  $mengajar->id) }}" method="POST">
            @method('PUT')
            @csrf
            <div class="mb-3">
                ...
                <select name="guru_id" id="guru_id" class="form-control @error('guru_id') error @enderror">
                    ...
                    <option value="{{ $guru->id }}" @if(old('guru_id', $mengajar->guru_id) == $guru->id) selected @endif required>{{ $guru->nama_guru}}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                ...
                <select name="mapel_id" id="mapel_id" class="form-control @error('mapel_id') error @enderror">
                    ...
                    <option value="{{ $mapel->id }}" @if(old('mapel_id', $mengajar->mapel_id) == $mapel->id) selected @endif required>{{ $mapel->nama_mapel}}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                ...
                <select name="kelas_id" id="kelas_id" class="form-control @error('kelas_id') error @enderror">
                    ...
                    <option value="{{ $kelas->id }}" @if(old('kelas_id', $mengajar->kelas_id) == $kelas->id) selected @endif required>{{ $kelas->nama_kelas}}</option>
                    @endforeach
                </select>
            </div>
